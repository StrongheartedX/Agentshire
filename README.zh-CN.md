...
> **你的 AI 团队，住在一座你亲手打造的 3D 游戏小镇里，而不是一个仪表盘。**
...
> **如果 Gateway 已在运行，需要重启才能加载插件**：
> ```bash
> openclaw gateway   # 或重启现有的 Gateway 进程
> ```
...
然后重启 Gateway，前端会自动重新构建。

---

## 使用方式

1. 安装插件后，**（重新）启动** OpenClaw Gateway
2. 小镇会自动在浏览器中打开
3. 在浏览器中对话——所有 Agent 活动会自动映射到小镇中
