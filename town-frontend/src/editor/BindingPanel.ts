...
const SLOTS: SlotDef[] = [
  { slot: 'office',   icon: SLOT_ICONS.office,   label: '办公室',    required: true,  auto: false },
  { slot: 'museum',   icon: SLOT_ICONS.museum,   label: '博物馆',    required: false, auto: false },
  { slot: 'userHome', icon: SLOT_ICONS.userHome,  label: '用户住宅',  required: false, auto: false },
  { slot: 'houses',   icon: SLOT_ICONS.houses,    label: 'NPC 住宅', required: false, auto: true  },
]
...
export class BindingPanel {
  ...
  refresh(): void {
    ...
    tip.textContent = '从下拉列表中选择建筑进行绑定'
  ...
  private renderSlotRow(def: SlotDef, bindings: typeof this.editor.config.bindings): HTMLElement {
    ...
    if (def.slot === 'houses') {
      ...
      desc.textContent = count > 0 ? `${count} 栋已绑定` : '从列表中选择建筑绑定'
      ...
      const addBtn = this.makeActionBtn('添加', 'bind', (e) => {
        e.stopPropagation()
      ...
      if (boundId) {
        ...
        const changeBtn = this.makeActionBtn('更换', 'change', (e) => {
          e.stopPropagation()
          this.showBuildingDropdown(def.slot, changeBtn)
        })
        actions.appendChild(changeBtn)

        const unbindBtn = this.makeActionBtn('解绑', 'unbind', (e) => {
          e.stopPropagation()
        ...
        desc.textContent = '未绑定'

        const bindBtn = this.makeActionBtn('绑定', 'bind', (e) => {
          e.stopPropagation()
